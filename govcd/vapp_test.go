...
func (vcd *TestVCD) Test_ChangeStorageProfile(test *C) {
	...
	test.Assert(err, IsNil)
	// Get StorageProfileReference
	storageprofileref, err := vcd.vdc.FindStorageProfileReference(vcd.config.VCD.StorageProfile.Storageprofile1)
	test.Assert(err, IsNil)
	// Compose VApp
	task, err := vcd.vdc.ComposeVApp(networks, vapptemplate, storageprofileref, "name", "description")
	test.Assert(err, IsNil)
	...
	task, err = vapp.ChangeStorageProfile(vcd.config.VCD.StorageProfile.Storageprofile2)
	test.Assert(err, IsNil)
	task.WaitTaskCompletion()
	test.Assert(task.Task.Status, Equals, "success")
	// Deleting VApp
