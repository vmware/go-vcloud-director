...
func (org *AdminOrg) CreateVdc(vdcConfiguration *types.VdcConfiguration) (AdminVdc, error) {
	...
	if err != nil {
		return AdminVdc{}, fmt.Errorf("error marshalling xml: %s", err)
	}
	xmlData := bytes.NewBufferString(xml.Header + string(output))
	util.Logger.Printf("[TRACE] AdminOrg.CreateVdc - xml payload: %s\n", xmlData)
	vdcCreateHREF, err := url.ParseRequestURI(org.AdminOrg.HREF)
	if err != nil {
		return AdminVdc{}, fmt.Errorf("error parsing admin org url: %s", err)
	}
	...
	resp, err := checkResp(org.client.Http.Do(req))
	if err != nil {
		return AdminVdc{}, fmt.Errorf("error instantiating a new VDC: %s", err)
	}

	adminVdc := NewAdminVdc(org.client)
	if err = decodeBody(resp, adminVdc.AdminVdc); err != nil {
		return AdminVdc{}, fmt.Errorf("error decoding admin VDC response: %s", err)
	}
	return *adminVdc, nil
}

// Creates the vdc and waits for the asynchronous task to complete.
func (org *AdminOrg) CreateVdcWait(vdcDefinition *types.VdcConfiguration) (AdminVdc, error) {
	adminVdc, err := org.CreateVdc(vdcDefinition)
	if err != nil {
		return AdminVdc{}, err
	}
	...
	if err != nil {
		return AdminVdc{}, fmt.Errorf("couldn't finish removing vdc %#v", err)
	}
	return adminVdc, nil
}
