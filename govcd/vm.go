...
// Deletes metadata (type MetadataStringValue) from the VM
...
func (vm *VM) DeleteMetadata(key string) (Task, error) {
	...
	req := vm.client.NewRequest(map[string]string{}, "DELETE", *apiEndpoint, nil)
...
// Adds metadata from type MetadataStringValue to the VM
// TODO: Support all MetadataTypedValue types with this function
func (vm *VM) AddMetadata(key, value string) (Task, error) {
	err := vm.Refresh()
	if err != nil {
		return Task{}, fmt.Errorf("error refreshing vm before running customization: %v", err)
	}
	...
	resp, err := checkResp(vm.client.Http.Do(req))
	if err != nil {
		return Task{}, fmt.Errorf("error customizing VM metadata: %s", err)
	}
