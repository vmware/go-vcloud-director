...
func (vcd *TestVCD) Test_GetAlbGeneralSettings(check *C) {
	...
	albSettings, err := edge.GetAlbGeneralSettings()
	check.Assert(err, IsNil)
	check.Assert(albSettings, NotNil)
	check.Assert(albSettings.Enabled, Equals, false)
}

func (vcd *TestVCD) Test_UpdateAlbGeneralSettings(check *C) {
	...
	enabledSettings, err := edge.UpdateAlbGeneralSettings(albSettingsConfig)
	check.Assert(err, IsNil)
	...
	disabledSettings, err := edge.UpdateAlbGeneralSettings(albSettingsConfig)
	check.Assert(err, IsNil)
	...
	enabledSettingsCustomServiceDefinition, err := edge.UpdateAlbGeneralSettings(albSettingsConfig)
	check.Assert(err, IsNil)
