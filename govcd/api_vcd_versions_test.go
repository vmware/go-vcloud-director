...
func (vcd *TestVCD) Test_GetVcdVersion(check *C) {
	if vcd.client.Client.APIVCDMaxVersionIs(">= 40.1") {
		check.Skip("Test is only relevant in VCD or VCFA 9.0")
	}
...
func (vcd *TestVCD) Test_GetVcfaVersion(check *C) {
	if vcd.client.Client.APIVCDMaxVersionIs("< 40.1") {
		check.Skip("Test is only relevant in VCFA 9.1+")
	}
	...
	// Comparing the current version against itself, without customer patch. Expected result: equal
	result, err := vcd.client.Client.VersionEqualOrGreater(version, 3)
	check.Assert(err, IsNil)
	check.Assert(result, Equals, true)
	...
	// Comparing the current version against itself, with build. Expected result: equal
	result, err = vcd.client.Client.VersionEqualOrGreater(version, 5)
	check.Assert(err, IsNil)
	check.Assert(result, Equals, true)

	digits[4] -= 1
	smallerBuild := intListToVersion(digits, 5)
	// Comparing the current version against same version, with smaller build. Expected result: greater
	result, err = vcd.client.Client.VersionEqualOrGreater(smallerBuild, 5)
	check.Assert(err, IsNil)
	check.Assert(result, Equals, true)

	// Comparing the current version against same version, with bigger build. Expected result: less
	digits[4] += 2
	biggerBuild := intListToVersion(digits, 5)
	result, err = vcd.client.Client.VersionEqualOrGreater(biggerBuild, 5)
	check.Assert(err, IsNil)
	check.Assert(result, Equals, false)
}
