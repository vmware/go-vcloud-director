...
// TestUploadFileRejectsEmptyFile pins the behaviour for a zero-length file.
// Iterating over offsets would skip the loop body entirely, so without an
// explicit check the function would report success without transferring
// anything, leaving the caller with an item that never received data. The
// previous implementation failed here with a bare io.EOF.
func TestUploadFileRejectsEmptyFile(t *testing.T) {
	...
	if err == nil {
		t.Fatal("upload of an empty file returned no error")
	}
	if uploadError == nil {
		t.Error("uploadError was not set although the upload failed")
	}
	if uploadSize != 0 {
		t.Errorf("uploadFile returned %d bytes for an empty file, expected 0", uploadSize)
	}
	if len(chunks) != 0 {
		t.Errorf("server received %d chunks for an empty file, expected none", len(chunks))
	}
}
