...
type OrgOAuthSettings struct {
	...
	// Added in v37.1
	EnableIdTokenClaims *bool `xml:"EnableIdTokenClaims,omitempty"` // Flag indicating whether Id-Token Claims should be used when establishing user details
	// Added in v38.0
	UsePKCE                                    *bool `xml:"UsePKCE,omitempty"`                                    // Flag indicating whether client must use PKCE (Proof Key for Code Exchange), which provides additional verification against potential authorization code interception. Default is false
	SendClientCredentialsAsAuthorizationHeader *bool `xml:"SendClientCredentialsAsAuthorizationHeader,omitempty"` // Flag indicating whether client credentials should be sent as an Authorization header when fetching the token. Default is false, which means client credentials will be sent within the body of the request
	// Added in v38.1
